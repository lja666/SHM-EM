...
COMPARISON = [
    ...
    },
    {
        ...
        "Predictive-SHM": "Not reported",
        "SHM-EM": "Yes",
        "basis": "Predictive-SHM reports standardized timestamped forecasts, but the primary source does not explicitly describe a common multi-model prediction origin or project-level synchronized future timeline. SHM-EM validates one batch origin and a common 40-step timeline.",
    },
    {
...
The controlled transition was exercised through a 15-case validation matrix comprising one positive control (P00), 12 failure-path cases (F01-F12), and two input-availability controls (I01-I02) in isolated databases. Persisted-result integrity was explicitly revalidated, and all cases expected to be blocked produced zero formal event, response-workflow, response-step, report, evidence, or prediction-link side effects. Evaluate remained side-effect free, while Execute reloaded the canonical series, recomputed and persisted the Gate, validated engineering units and rule semantics, and only then created formal records. One captured event trace resolves the event to rule version v2, prediction batch 40, run 236, the settlement model artifact hash, its input window and schema hash, a 40-step forecast snapshot, Gate 1, the first exceedance, and the response workflow. The reproduction database was restored after export.
...
FACTS = {
    "R1-0": ("Retained the architecture and redirected the revision to software validation.", "Contract, Future State, failure, reuse, runtime, provenance, and portability evidence packages.", "Introduction revision summary and expanded validation section.", "No new forecasting-algorithm comparison.", "55 backend tests; 13 PIT_PRE tests; a 15-case validation matrix (P00, F01-F12, I01-I02); 7 reuse checks."),
    "R1-1": ("Added one synthetic bridge-monitoring configuration and a registration/change inventory.", "Phase 1B functional B9-B15 regression on the performance-corrected frozen core.", "Section 3 cross-configuration reuse and Section 4.2.", "No external field validation or predictive generalization.", "3 stations, 12 instruments, 2 compatible model bundles used solely as software-workflow fixtures, 1,120 forecast rows; B9-B15 = 7/7."),
    ...
    "R1-3": ("Added isolated failure-path and persisted-integrity testing and rechecked execution eligibility.", "A 15-case validation matrix comprising one positive control, 12 failure-path cases, and two input-availability controls.", "Section 3 failure-path validation and sequence figure.", "No absolute safety claim beyond tested cases.", "All cases expected to be blocked produced zero formal side effects."),

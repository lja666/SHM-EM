...
"""
============================================================
水位 water 未来预测代码：固定最优参数 + 直接预测未来
============================================================

用途：
    输入平台过去监测数据，直接得到未来 water_value 预测数据。
...
给平台开发人员看的最简说明：
    ...
    6. 把 MODEL_PATH 改成 best_model_water_optuna.pth 的实际路径；
...
整体输入输出：
    输入：
        平台过去数据 CSV
        best_model_water_optuna.pth
        best_params.json

    输出：
        water_future_prediction_wide.csv
        water_future_prediction_long.csv
...
重要：
    ...
    不是 YD，不是 XD，也不是 Strain / Pressure。
"""
...
#
# 注意：
#   这里必须是用 water_value 作为目标训练出来的模型。
#   不能用 YD/XD/Strain/Pressure 模型替代。
#
# 推荐部署方式：
#   把本代码、best_model_water_optuna.pth、best_params.json
#   放在同一个文件夹里。
MODEL_PATH = r"./best_model_water_optuna.pth"
...
#   适合写入健康监测平台数据库。
OUTPUT_WIDE_PATH = r"./water_future_prediction_wide.csv"
OUTPUT_LONG_PATH = r"./water_future_prediction_long.csv"
...
def fit_scalers(df, input_cols, water_cols):
    """
    按训练代码方式拟合 scaler。
    ...
    为什么预测时还要 scaler？
        ...
        模型输出后，还要用 scaler_response 把结果变回真实 water 数值。
    ...
    scaler_response.fit(df_final_train[water_cols])
...
def build_latest_input(df, input_cols, water_cols, scaler_all, params):
    """
    ...
    对 water 模型：
        x_response = 最近 m 行 water
        x_cat      = 最近 m 行 YD + XD + Strain + Pressure
        x_env      = water 滞后输入

    也就是说：
        water 是预测目标；
        YD、XD、Strain、Pressure 都作为辅助输入特征参与预测。
    """
    ...
    # 前 lag 行用于 water 滞后；
    # 后 m 行用于主输入。
    x_response_np = values[lag:lag + m, idx_env_start:idx_env_end]
...
def make_output(pred_inverse, water_cols, latest_time, latest_time1, params):
    """
    把模型预测结果整理成两个 CSV。
    ...
    for step in range(1, n + 1):
        ...
        for j, col in enumerate(water_cols):
    ...
    for step in range(1, n + 1):
        ...
        for j, col in enumerate(water_cols):
            long_rows.append({
                ...
                "point": col,
                "water_pred": pred_inverse[step - 1, j]
            })
...
def main():
    ...
    主程序执行顺序：
        ...
        2. 加载 best_model_water_optuna.pth；
        3. 根据模型权重校正参数；
        4. 读取平台过去数据；
        5. 拟合 scaler；
        6. 取最近 m+lag 行数据；
    ...
    actual_response_dim = len(water_cols)
    ...
    if actual_response_dim != expected_response_dim:
        raise ValueError(
            f"water列数和模型输出不一致：数据中 {actual_response_dim} 列，"
    ...
    print("\n5. 按原训练方式拟合 scaler...")
    scaler_all, scaler_response = fit_scalers(df, input_cols, water_cols)

    print("\n6. 构造最近 m+lag 行输入...")
    x_response, x_env, x_cat, latest_time, latest_time1 = build_latest_input(
        df, input_cols, water_cols, scaler_all, params
    )
    ...
    wide_df, long_df = make_output(pred_inverse, water_cols, latest_time, latest_time1, params)

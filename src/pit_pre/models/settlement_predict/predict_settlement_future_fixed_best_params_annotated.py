...
"""
============================================================
沉降 settlement 未来预测代码：固定最优参数 + 直接预测未来
============================================================

用途：
    输入平台过去监测数据，直接得到未来 settlement_value 预测数据。

给平台开发人员看的最简说明：
    1. 不要运行训练代码；
    2. 不要重新贝叶斯优化；
    3. 不要重新训练；
    4. 不要运行 m10n3、m20n3、m30n3、m40n3、m50n3 对比实验；
    5. 只需要运行本文件；
    6. 把 DATA_PATH 改成平台导出的历史监测数据 CSV；
    7. 把 MODEL_PATH 改成 best_model_settlement_optuna.pth 的实际路径；
    8. 把 BEST_PARAMS_PATH 改成 best_params.json 的实际路径；
    9. 运行后会生成两个预测结果 CSV。

整体输入输出：
    输入：
        平台过去数据 CSV
        best_model_settlement_optuna.pth
        best_params.json

    输出：
        settlement_future_prediction_wide.csv
        settlement_future_prediction_long.csv
...
重要：
    ...
    不是 YD，不是 XD，不是 Strain，不是 Pressure，也不是 water。
"""
...
#   必须包含 settlement_value；
#   如果有 settlementbaseline_value、settlementdelta_value、
#   settlementdata2_value、settlementtemperature_value，则会作为辅助输入。
#
# 程序会自动按 time1 排序，然后取最后 m+lag 行进行未来预测。
#
# 当前上传 best_params.json 为：
#   m = 10
#   lag = 2
#
# 因此平台输入数据至少需要最近 12 行连续数据。
# 也可以给完整历史数据，程序会自动取最后 13 行。
DATA_PATH = r"input_wide.csv"

# 已训练好的 settlement 最优模型路径。
#
# 注意：
#   这里必须是用 settlement_value 作为目标训练出来的模型。
#   不能用 YD/XD/Strain/Pressure/water 模型替代。
#
# 推荐部署方式：
#   把本代码、best_model_settlement_optuna.pth、best_params.json
#   放在同一个文件夹里。
MODEL_PATH = r"./best_model_settlement_optuna.pth"

# settlement 模型训练时保存的最优参数。
#
# 这个文件提供：
#   m
#   lag
...
#   等模型构造时需要的参数。
#
# 如果 best_params.json 里没有 n，本代码会从模型权重 fc 层自动识别 n。
...
# wide:
#   一行表示一个未来预测步；
#   每列表示一个 settlement 测点预测值；
#   适合直接用 Excel 查看。
#
# long:
#   一行表示一个未来时间点下某个测点的预测值；
#   适合写入健康监测平台数据库。
OUTPUT_WIDE_PATH = r"./settlement_future_prediction_wide.csv"
OUTPUT_LONG_PATH = r"./settlement_future_prediction_long.csv"
...
class TransformerCnn(nn.Module):
    def __init__(self, response_dim, env_dim, raw_trans_dim, attn_dim,
        ...
        # 多源结构输入：
        # YD + XD + Strain + Pressure + settlement_value + settlement辅助列
        # 先映射到注意力维度。
        ...
        # 三个 CNN 分支：
        #   conv_response: 历史 settlement_value
        #   conv_env     : 滞后 water
...
def load_best_params(path):
    ...
    注意：
        本次 settlement best_params.json 中保存的是：
            m
            lag
            attn_dim
            num_heads
        ...
        其中 learning_rate、batch_size 是训练时使用的，直接预测时不会再用。
    """
    path = Path(path)
...
def infer_params_from_best_model(state, params):
    ...
    if "fc.weight" in state and "response_dim" in params:
        ...
        # 所以先使用 best_params 中的 m 和 lag，再检查是否匹配。
...
def read_data_and_columns(data_path):
    ...
    模型真正输入列顺序必须保持：
        YD + XD + Strain + Pressure + settlement + settlement_aux + water
    ...
    # 与本次训练代码保持完全相同的 10 个沉降测点及顺序
    settlement_point_prefixes = [
        "1point1",
        "1point2",
        "1point3",
        "1point4",
    ...
    settlement_cols = [
        f"{point}settlement_value"
        for point in settlement_point_prefixes
    ]

    settlement_baseline_cols = [
        f"{point}settlementbaseline_value"
        for point in settlement_point_prefixes
    ]
    settlement_delta_cols = [
        f"{point}settlementdelta_value"
        for point in settlement_point_prefixes
    ]
    settlement_data2_cols = [
        f"{point}settlementdata2_value"
        for point in settlement_point_prefixes
    ]
    settlement_temperature_cols = [
        f"{point}settlementtemperature_value"
        for point in settlement_point_prefixes
    ]
    ...
    required_settlement_cols = settlement_cols + settlement_aux_cols
    missing_settlement_cols = [
        col for col in required_settlement_cols
        if col not in df.columns
    ]
    if missing_settlement_cols:
        raise ValueError(
            "输入数据缺少以下沉降字段：\n" +
            "\n".join(missing_settlement_cols)
        )
    ...
    if len(settlement_cols) == 0:
        raise ValueError("未识别到 settlement_value 目标列。")
...
def fit_scalers(df, input_cols, settlement_cols):
    """
    按训练代码方式拟合 scaler。
    ...
    为什么预测时还要 scaler？
        ...
        模型输出后，还要用 scaler_response 把结果变回真实 settlement 数值。
    ...
    scaler_response.fit(df_final_train[settlement_cols])
...
def build_latest_input(df, input_cols, settlement_cols, water_cols, scaler_all, params):
    """
    ...
    对 settlement 模型：
        x_response = 最近 m 行 settlement_value
        x_cat      = 最近 m 行 YD + XD + Strain + Pressure + settlement + settlement_aux
        x_env      = water 滞后输入

    也就是说：
        settlement 是预测目标；
        YD、XD、Strain、Pressure、water 和 settlement 辅助列都参与预测。
    """
    ...
    settlement_start = input_cols.index(settlement_cols[0])
    settlement_end = settlement_start + len(settlement_cols)

    env_start = input_cols.index(water_cols[0])
    env_end = len(input_cols)

    # latest_df 总长度 m+lag：
    # 前 lag 行用于 water 滞后；
    # 后 m 行用于主输入。
    x_response_np = values[lag:lag + m, settlement_start:settlement_end]
    x_cat_np = values[lag:lag + m, 0:env_start]
    x_env_np = values[m:m + lag, env_start:env_end]
...
def make_output(pred_inverse, settlement_cols, latest_time, latest_time1, params):
    """
    把模型预测结果整理成两个 CSV。

    wide_df：
        一行 = 一个未来预测步；
        每列 = 一个 settlement 测点预测值；
        适合人工查看或 Excel 打开。
    ...
    for step in range(1, n + 1):
        ...
        for j, col in enumerate(settlement_cols):
    ...
    for step in range(1, n + 1):
        ...
        for j, col in enumerate(settlement_cols):
            long_rows.append({
                ...
                "point": col,
                "settlement_pred": pred_inverse[step - 1, j]
            })
...
def main():
    ...
    主程序执行顺序：
        ...
        2. 加载 best_model_settlement_optuna.pth；
        3. 根据模型权重校正参数；
        4. 读取平台过去数据；
        5. 拟合 scaler；
        6. 取最近 m+lag 行数据；
    ...
    (
        ...
        water_cols,
        input_cols
    ) = read_data_and_columns(DATA_PATH)
    ...
    if actual_response_dim != expected_response_dim:
        raise ValueError(
            f"settlement目标列数和模型输出不一致：数据中 {actual_response_dim} 列，"
            f"模型需要 {expected_response_dim} 列。"
        )

    if actual_env_dim != expected_env_dim:
        raise ValueError(
            f"water列数和环境输入不一致：数据中 {actual_env_dim} 列，"
            f"模型需要 {expected_env_dim} 列。"
        )

    if actual_raw_trans_dim != expected_raw_trans_dim:
        raise ValueError(
            f"YD+XD+Strain+Pressure+settlement+settlement_aux 列数和模型不一致："
            f"数据中 {actual_raw_trans_dim} 列，模型需要 {expected_raw_trans_dim} 列。"
        )

    print("\n5. 按原训练方式拟合 scaler...")
    scaler_all, scaler_response = fit_scalers(df, input_cols, settlement_cols)

    print("\n6. 构造最近 m+lag 行输入...")
    x_response, x_env, x_cat, latest_time, latest_time1 = build_latest_input(
        df, input_cols, settlement_cols, water_cols, scaler_all, params
    )
    ...
    wide_df, long_df = make_output(pred_inverse, settlement_cols, latest_time, latest_time1, params)
